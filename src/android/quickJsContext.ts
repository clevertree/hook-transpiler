...
export function createQuickJsContext(evalFn?: typeof eval): QuickJsModuleContext {
    ...
    return {
        executeCode(code: string, filename = 'module.jsx'): any {
            try {
...
`;
                ...
                (globalThis as any).__quickjs_context_globals = globals

                try {
                    const result = $eval(wrappedCode)
                    // After execution, update our module reference
                    module = (globalThis as any).__quickjs_context_module || module
                    return result
                } finally {
                    delete (globalThis as any).__quickjs_context_globals
                }
            } catch (error) {
        ...
        getGlobal(name: string): any {
            return globals[name] ?? (globalThis as any)[name]
        },
 ...
 */
export function formatTranspiledCode(code: string, maxLines = 10): string {

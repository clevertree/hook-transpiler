...
| `input` | `padding: 8dp 12dp`, `borderRadius: 4dp`, `borderWidth: 1dp`, `minHeight: 40dp`, `placeholderColor: color-mix(...)` |
| `select` | `padding: 8dp 12dp`, `borderRadius: 4dp`, `borderWidth: 1dp`, `minHeight: 40dp` |
| `textarea` | `padding: 12dp`, `borderRadius: 4dp`, `borderWidth: 1dp`, `minHeight: 80dp` |

## Advanced Color Features

The styling engine supports dynamic color resolution:

### 1. `currentColor`
References the `color` property of the current element.
```yaml
".input-border":
  color: "#3b82f6"
  borderColor: "currentColor" # Resolves to #3b82f6
```

### 2. `color-mix`
Mixes two colors in a specific color space (currently `srgb`).
```yaml
"input":
  color: "var(text)"
  # Creates a 50% transparent version of the text color for the placeholder
  placeholderColor: "color-mix(in srgb, currentColor 50%, transparent)"
```

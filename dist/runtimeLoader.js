...
export class WebModuleLoader {
    async executeModule(code, filename, context, fetchUrl) {
        ...
        try {
            ...
            if (looksLikeESM) {
                try {
                    // Transpiler has already rewritten special imports (react, @clevertree/*, etc.) to use globals
                    // Store module metadata in globalThis for @clevertree/meta import (ES6 compliant)
                    ...
                    const createJsxFactory = (React) => {
                        ...
                        const logJsx = (kind, type, key, props) => {
                            if (!shouldLogJsx)
                                return;
                            try {
...
export class RNModuleLoader {
    ...
    async executeModule(code, filename, context, fetchUrl) {
        ...
        if (usesES6Import && !this.importHandler) {
            console.warn('[RNModuleLoader] Code uses import() but no ES6ImportHandler available. Install will fail.', { filename });
        }
        // eslint-disable-next-line no-new-func
...
//# sourceURL=${filename}
    `);
        ...
        console.log('[RNModuleLoader] module.exports === exports?', module.exports === exports);
            ...
            if (shouldTranspile) {
                try {
                    if (this.transpiler) {
                        ...
                        finalCode = await transpileCode(code, { filename: normalizedPath }, false // Web uses import, not CommonJS
                        );
                    }
                }
                catch (err) {

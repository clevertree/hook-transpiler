...
function transformOnce(input) {
    let code = input;
    let changed = false;
    // Self-closing tags first: <Tag a={1} />
    code = code.replace(/<([A-Za-z_][\w\.]*)\s*([^>]*)\/>/g, (_m, tag, attrs) => {
        changed = true;
    ...
    code = code.replace(/<([A-Za-z_][\w\.]*)\s*([^>]*)>([\s\S]*?)<\/\1>/g, (_m, tag, attrs, body) => {
        changed = true;
        const props = parseProps(attrs || '');
        ...
        while (rest.length) {
            ...
            if (t)
                children.push(JSON.stringify(t));
...
export function transformJsxToJs(sourceCode, filename = 'module.jsx') {
    ...
    try {
        ...
        const e = new Error(`TranspileError: ${filename}: ${err?.message || String(err)}`);
        e.name = 'TranspileError';
        throw e;
    }
}
export default transformJsxToJs;
//# sourceMappingURL=simpleJsx.js.map